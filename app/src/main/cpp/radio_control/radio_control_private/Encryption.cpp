...
bool blowfishEncrypt(const ByteArray &key, ByteArray *data) {
  ...
  blowfish_key_setup((unsigned char *)key.data(), &keystruct, key.size());

  int block_size = BLOWFISH_BLOCK_SIZE;

  while ((data->size() % block_size) != 0)
    data->append((char)0xFF);

  unsigned char *block = reinterpret_cast<unsigned char *>(data->data());
  int numberOfBlocks = int(data->size()) / block_size;
...
bool blowfishDecrypt(const ByteArray &key, ByteArray *data,
                     size_t unencryptedSize) {
  if (key.isEmpty()) {
    TRACE("No key");
    return false;
  }
  ...
  int padding = (int)data->size() - (int)unencryptedSize;

  // Strictly speaking, padding should be < BLOCK_SIZE, but
  // in some early code it could be up to BLOCK_SIZE
  if (padding < 0 || padding > (int)block_size) {
    TRACE("Illegal padding");
    return false;
  }

  if ((data->size() % block_size) != 0) {
    TRACE("Bad encrypted size");
    return false;
  }
  ...
  for (int n = 0; n < numberOfBlocks; n++) {
    blowfish_decrypt(block, block, &keystruct);
    block += block_size;
  }

  data->resize(unencryptedSize);
